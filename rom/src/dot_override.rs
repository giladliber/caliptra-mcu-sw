...
impl<'a> RecoveryTransport for Mbox0RecoveryTransport<'a> {
    fn wait_for_override_request(&self) -> McuResult<crate::OverrideRequest<'_>> {
        ...
        let ecc_pub_key = EccP384PublicKey {
            x: req.ecc_pub_key_x.into(),
            y: req.ecc_pub_key_y.into(),
        };
    ...
    fn receive_override_response(&self) -> McuResult<crate::OverrideChallengeResponse<'_>> {
        ...
        let resp = session
            ...
            .ok_or(McuError::ROM_DOT_OVERRIDE_CHALLENGE_FAILED)?;
        let ecc_pub_key = EccP384PublicKey {
            x: resp.ecc_pub_key_x.into(),
            y: resp.ecc_pub_key_y.into(),
        };
        let ecc_signature_r = resp.ecc_sig_r.to_bytes();
        let ecc_signature_s = resp.ecc_sig_s.to_bytes();
